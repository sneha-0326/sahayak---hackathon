// ── Questions (20 clinically-validated fistula screening questions) ─────────
const QUESTIONS = {
  'en-IN': [
...
function saveWomen() { /* data stored in MongoDB */ }
...
// ── Risk Calculator (fallback) ─────────────────────────────────────────────
function calcRisk(answers) {
  const score = answers.reduce((s, a) => {
    if (a === 'yes') return s + 2;
    if (a === 'sometimes' || a === 'notsure') return s + 1;
    return s;
  }, 0);
...
function screenLanding() {
  return `
    ...
    <div class="landing-features">
      <div class="landing-feature-item"><span>🏥</span><div><strong>Early Detection</strong><p>Screen for obstetric fistula with culturally sensitive questions</p></div></div>
      <div class="landing-feature-item"><span>🔊</span><div><strong>Audio & Self Mode</strong><p>Works for all literacy levels in Hindi & English</p></div></div>
      <div class="landing-feature-item"><span>📍</span><div><strong>Find Nearby Hospitals</strong><p>Instantly locate hospitals based on patient location</p></div></div>
      <div class="landing-feature-item"><span>🔒</span><div><strong>Private & Secure</strong><p>All answers are confidential and securely stored</p></div></div>
    </div>
...
function screenLogin() {
  ...
  return `
    ...
    <div class="form-group"><label>Email</label><input type="email" id="l-email" placeholder="Enter email" /></div>
    <div class="form-group"><label>Password</label><input type="password" id="l-password" placeholder="Enter password" /></div>
...
async function doLogin(role) {
  ...
  } catch (e) { err.textContent = 'Could not connect to server. Please try again.'; }
}
...
function screenSignup() {
  return `
    ...
    <div class="form-group"><label>Full Name</label><input type="text" id="s-name" placeholder="Enter your name" /></div>
    <div class="form-group"><label>Email</label><input type="email" id="s-email" placeholder="Enter email" /></div>
    <div class="form-group"><label>Phone Number</label><input type="tel" id="s-phone" placeholder="Enter phone number" /></div>
    <div class="form-group"><label>Password</label><input type="password" id="s-password" placeholder="Create password" /></div>
    <div class="form-group"><label>Confirm Password</label><input type="password" id="s-confirm" placeholder="Confirm password" /></div>
...
async function startWomanScreening() {
  ...
  try {
    ...
    if (!res.ok) { alert('Please add your details first.'); navigate('add-woman'); return; }
...
function screenDashboard() {
  return `
    ...
    <div class="feature-grid">
      <div class="feature-card"><div class="feature-icon">📋</div><h4>Patient Records</h4><p>Register and manage women's profiles</p></div>
      <div class="feature-card"><div class="feature-icon">🔊</div><h4>Dual Screening Modes</h4><p>Audio or self-read mode in Hindi & English</p></div>
      <div class="feature-card"><div class="feature-icon">📊</div><h4>ML Risk Assessment</h4><p>AI-powered risk scoring using 20 clinical questions + vitals</p></div>
      <div class="feature-card"><div class="feature-icon">🏥</div><h4>Hospital Finder</h4><p>Locate nearby hospitals for high-risk cases</p></div>
    </div>
    <button class="btn btn-outline" style="margin-top:20px;" onclick="logout()">🚪 Logout</button>
  `;
}
  ...
  const items = state.women.length
    ? state.women.map((w, i) => `
        <button class="woman-item" onclick="navigate('profile', {selectedWoman: ${i}})">
          ...
          <div class="woman-info"><strong>${w.name}</strong><span>Age: ${w.age}</span></div>
        </button>`).join('')
...
function screenAddWoman() {
  ...
  return `
    ...
    <div class="form-group"><label>Full Name</label><input type="text" id="f-name" placeholder="Enter name" /></div>
    <div class="form-group"><label>Age</label><input type="number" id="f-age" placeholder="Enter age" min="10" max="80" /></div>
    <div class="form-group"><label>Phone Number</label><input type="tel" id="f-phone" placeholder="Enter phone number" /></div>
    <div class="form-group"><label>State</label><input type="text" id="f-state" placeholder="Enter state" /></div>
    <div class="form-group"><label>District</label><input type="text" id="f-district" placeholder="Enter district" /></div>
    <div class="form-group"><label>City / Village</label><input type="text" id="f-city" placeholder="Enter city or village" /></div>
...
async function saveWoman() {
  ...
  if (!name || !age || !phone || !state_ || !district || !city) { err.textContent = 'Please fill in all fields.'; return; }
  try {
    ...
    await loadWomen();
    navigate(state.currentUser?.role === 'woman' ? 'woman-dashboard' : 'women-list');
...
function screenProfile() {
  ...
  return `
    ...
    <div class="profile-avatar"><div class="avatar-circle">👩</div><h2>${w.name}</h2></div>
...
function screenLangSelect() {
  ...
  return `
    ...
    <div class="lang-grid">
      ${LANGUAGES.map(l => `
        <button class="lang-btn ${state.selectedLang === l.code ? 'lang-btn-active' : ''}" onclick="selectLang('${l.code}')">
...
function selectLang(code) { state.selectedLang = code; render(); }
...
function screenModeSelect() {
  return `
    ...
    <div class="mode-card" onclick="startScreening(true)">
      <div class="mode-icon">🔊</div><h3>Audio Mode</h3><p>Questions will be read aloud in selected language</p>
    </div>
    <div class="mode-card" onclick="startScreening(false)">
      <div class="mode-icon">👁️</div><h3>Self Mode</h3><p>Read questions on screen in selected language</p>
    </div>
  `;
}
...
function screenComfortIntro() {
  ...
  const msg = isHindi
    ? 'प्रसव के बाद कई महिलाओं को ये समस्याएं होती हैं। यह सामान्य है और इसका इलाज संभव है। आपके जवाब पूरी तरह निजी हैं।'
    : 'Many women face these problems after delivery. It is normal and treatable. Your answers are completely private.';
...
function beginQuestions() { navigate('question'); if (state.audioMode) speakQuestion(); }
...
function goBackQuestion() {
  if (state.currentQ > 0) { state.currentQ--; state.answers.pop(); render(); if (state.audioMode) speakQuestion(); }
  else { navigate('comfort-intro'); }
}

function answer(val) {
  state.answers.push(val);
  if (state.currentQ + 1 < state.questions.length) {
    state.currentQ++; render(); if (state.audioMode) speakQuestion();
  } else { navigate('vitals'); }
}
...
function screenVitals() {
  ...
  return `
    ...
    <p style="color:#555;font-size:14px;margin-bottom:20px;text-align:center;">Enter health readings for ML-based risk prediction</p>
    <div class="form-group"><label>Age</label><input type="number" id="v-age" placeholder="e.g. 25" value="${w?.age || ''}" min="10" max="80" /></div>
    <div class="form-group"><label>Systolic Blood Pressure (mmHg)</label><input type="number" id="v-sbp" placeholder="e.g. 120" min="70" max="200" /></div>
    <div class="form-group"><label>Diastolic Blood Pressure (mmHg)</label><input type="number" id="v-dbp" placeholder="e.g. 80" min="40" max="130" /></div>
    <div class="form-group"><label>Blood Sugar (mmol/L)</label><input type="number" id="v-bs" placeholder="e.g. 7.5" step="0.1" min="1" max="30" /></div>
    <div class="form-group"><label>Body Temperature (°F)</label><input type="number" id="v-temp" placeholder="e.g. 98.6" step="0.1" min="95" max="105" /></div>
    <div class="form-group"><label>Heart Rate (bpm)</label><input type="number" id="v-hr" placeholder="e.g. 76" min="40" max="200" /></div>
...
async function submitVitals() {
  ...
  if (!age || !sbp || !dbp || !bs || !temp || !hr) { err.textContent = 'Please fill in all fields.'; return; }
  err.textContent = '';
...
function screenResult() {
  ...
  return `
    ...
    ${hospitalBtn}
    <div class="reassurance-box">💛 This is just a health check. Many women face these issues. Help is available and treatment is possible.</div>
...
function initMap() {
  ...
  if (typeof showHospitalMap === 'function') {
    ...
    document.getElementById('map-container').innerHTML = '<p style="padding:20px;text-align:center;color:#888;">Map component not loaded.</p>';
  }
}
